...
- ~~Cancelled Google sign-in (`?error=access_denied`) shows raw JSON 401 — redirect to `/` instead (more common now that the account chooser always shows).~~ (done in Plan 5)
...
- ~~`jwt.verify` with explicit `algorithms: ['HS256']`.~~ (done in Plan 5)
- ~~`safeReturnPath` accepts dot segments (`/app/../x`) — same-origin, but escapes `/app`.~~ (done in Plan 5)
- Clear `oauth_state` on 401 paths and `return_to` on 403.
- Error handler throws on a literal `null`/`undefined` error value.
- ~~`permissions: contents: read` on the `test`/`e2e` PR-check jobs.~~ (done in Plan 5)
- ~~`.dockerignore`: exclude `.superpowers/`.~~ (done in Plan 5)
- Sidebar modified-click guard: add `altKey`.
- ~~Camera picker chevron colour is baked into a data URI (doesn't re-tint in light theme).~~ (done in Plan 5)
